...
pub(crate) async fn amap_download(
    ...
    let manga_detail = purchase_results.comic();
    ...
    if results.is_empty() {
        return Err(color_eyre::eyre::eyre!(
            "No chapters to be download after filtering, aborting"
        ));
    }
    ...
    for chapter in download_chapters {
        ...
        let ch_view = match client.get_comic_viewer(title_id, &consume).await {
            Ok(ch_view) => ch_view,
            Err(err) => {
