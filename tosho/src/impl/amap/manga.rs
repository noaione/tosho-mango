    ...
    if show_chapters {
        for episode in info.episodes().iter() {
            ...
            if episode.info().price() > 0 {
                base_info = cformat!("{} [<r!,strong>{}</>T]", base_info, episode.info().price());
                ...
                base_info = cformat!("{} <b!,strong>[<rev>Free</rev>]</b!,strong>", base_info);
            }
            console.info(&base_info);
            if let Some(update_at) = unix_timestamp_to_string(episode.info().update_date() as i64) {
