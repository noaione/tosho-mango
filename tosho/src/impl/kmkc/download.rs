...
use crate::r#impl::models::{ChapterDetailDump, MangaDetailDump};
...
pub(crate) async fn kmkc_download(
    ...
    let title_detail = purchase_results.title();
    let user_point = purchase_results.balance();
    let all_chapters = purchase_results.all_episodes();
                ...
                if ticket_entry.is_title_available() {
                    ...
                    ticket_info = Some(TicketInfoType::Premium(
                        ticket_entry
                            .info()
                            .premium()
                            .ok_or_eyre("Failed to get ticket premium information")?,
                    ));
    ...
    if download_chapters.is_empty() {
        ...
        return Err(color_eyre::eyre::eyre!(
            "No chapters to download after filtering, aborting"
        ));
    }
    ...
    for chapter in download_chapters {
        ...
        let viewer_info = match client.get_episode_viewer(chapter).await {
            Ok(info) => info,
            Err(e) => {
        ...
        let image_dir = get_output_directory(&output_dir, title_id, Some(chapter.id()), false)?;
