...
pub async fn sjv_account_login(
    ...
    let (account, instance_id) = SJClient::login(&email, &password, act_mode, sj_platform)
        .await
        .context("Failed to authenticate")?;

    let config: SJConfig = SJConfig::from_login_response(&account, instance_id, sj_platform);
...
pub(crate) fn sjv_account_revoke(
    account: &Config,
    console: &crate::term::Terminal,
) -> color_eyre::Result<()> {
