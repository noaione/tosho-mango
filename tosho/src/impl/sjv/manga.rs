...
pub(crate) async fn sjv_search(
    ...
    if results.series.is_empty() {
        ...
        return Err(color_eyre::eyre::eyre!("No series exist"));
    }
    ...
    if filtered.is_empty() {
        ...
        return Err(color_eyre::eyre::eyre!("No match found"));
    }
...
pub(crate) async fn sjv_title_info(
    ...
    if show_chapters && result.total_chapters() > 0 {
        ...
        match chapters_info {
            ...
            Ok(series_response) => {
                let mut chapters_info: Vec<tosho_sjv::models::MangaChapterDetail> = series_response
                    .chapters()
                    .iter()
    ...
    if !chapters_lists.is_empty() {
        for chapter in chapters_lists {
            ...
            if chapter.is_available() {
                base_txt = cformat!("{} <g!,strong>[<rev>Free</rev>]</g!,strong>", base_txt);
            }
            ...
            if let Some(expiry_at) = chapter.expiry_at() {
                let expiry_at = unix_timestamp_to_string(expiry_at).unwrap_or("N/A".to_string());
